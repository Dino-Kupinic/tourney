...
COMMENT ON TYPE "public"."registration_status" IS 'Status der Anmeldung';
...
COMMENT ON TYPE "public"."sport_type" IS 'sportarten';
...
COMMENT ON TYPE "public"."tournament_phase" IS 'Gruppenphase etc.';



CREATE OR REPLACE FUNCTION "public"."generate_final"("p_tournament_id" "uuid", "p_start_time" time without time zone, "p_interval_minutes" integer) RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  team1             UUID;
...
CREATE OR REPLACE FUNCTION "public"."generate_group_stage_matches"("p_tournament_id" "uuid", "p_start_time" time without time zone, "p_interval_minutes" integer) RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  team1 UUID;
...
CREATE OR REPLACE FUNCTION "public"."generate_quarterfinals"("p_tournament_id" "uuid", "p_start_time" time without time zone, "p_interval_minutes" integer) RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  team1              UUID;
...
CREATE OR REPLACE FUNCTION "public"."generate_semifinals"("p_tournament_id" "uuid", "p_start_time" time without time zone, "p_interval_minutes" integer) RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  team_ids         UUID[];
...
CREATE OR REPLACE FUNCTION "public"."record_match_result"("p_match_id" "uuid", "p_team1_score" smallint, "p_team2_score" smallint) RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  winner UUID;
...
COMMENT ON TABLE "public"."class" IS 'Schulklasse';
...
COMMENT ON TABLE "public"."group" IS 'Gruppenphase eines Turniers';
...
COMMENT ON TABLE "public"."match" IS 'Match zwischen zwei Teams';
...
COMMENT ON TABLE "public"."result" IS 'Ergebnis eines Matches';
...
COMMENT ON TABLE "public"."team" IS 'Ein Team';
...
COMMENT ON TABLE "public"."logo" IS 'auswählbares Teamlogo';
...
ALTER TABLE "public"."logo_variant" OWNER TO "postgres";


COMMENT ON TABLE "public"."logo_variant" IS 'Farbvariante eines Teamlogos';
...
ALTER TABLE "public"."matches_status" OWNER TO "postgres";
...
COMMENT ON TABLE "public"."player" IS 'Schüler';



COMMENT ON COLUMN "public"."player"."note" IS 'Notiz';
...
COMMENT ON TABLE "public"."registration" IS 'Anmeldungen für Turniere';
...
CREATE TABLE IF NOT EXISTS "public"."tournament" (
    ...
    CONSTRAINT "prizes_json_schema" CHECK ((("prizes" IS NULL) OR (("prizes" ? 'first'::"text") AND (("prizes" ->> 'first'::"text") IS NOT NULL) AND ("prizes" ? 'second'::"text") AND (("prizes" ->> 'second'::"text") IS NOT NULL) AND ("prizes" ? 'third'::"text") AND (("prizes" ->> 'third'::"text") IS NOT NULL) AND ("prizes" ? 'bonus'::"text") AND (("prizes" ->> 'bonus'::"text") IS NOT NULL))))
);
...
COMMENT ON TABLE "public"."tournament" IS 'Repräsentiert ein gesamtes Turnier (Fußballturnier, Volleyballturnier, ...)';



COMMENT ON COLUMN "public"."tournament"."prizes" IS 'Turnierpreise';



COMMENT ON COLUMN "public"."tournament"."thumbnail_path" IS 'Path to the thumbnail, Based on Supabase Storage';



COMMENT ON COLUMN "public"."tournament"."location" IS 'Ort des Turniers';



COMMENT ON COLUMN "public"."tournament"."groups" IS 'Anzahl an Gruppen';



COMMENT ON COLUMN "public"."tournament"."group_teams" IS 'Anzahl an Teams pro Gruppe';
...
CREATE OR REPLACE TRIGGER "trigger_final_results" AFTER INSERT OR UPDATE ON "public"."result" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_update_tournament_results"();



CREATE OR REPLACE TRIGGER "trigger_group_stage_results" AFTER INSERT OR UPDATE ON "public"."result" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_generate_quarterfinals"();



CREATE OR REPLACE TRIGGER "trigger_quarterfinal_results" AFTER INSERT OR UPDATE ON "public"."result" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_generate_semifinals"();



CREATE OR REPLACE TRIGGER "trigger_semifinal_results" AFTER INSERT OR UPDATE ON "public"."result" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_generate_final"();
